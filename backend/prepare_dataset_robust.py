...
def _parse_label_line(line: str) -> tuple[bool, int | None, str | None, bool]:
    parts = line.strip().split()
    if len(parts) != 5 and (len(parts) < 7 or len(parts) % 2 == 0):
        return False, None, "numero_de_campos_invalido", False

    try:
        cid = int(parts[0])
        coords = [float(v) for v in parts[1:]]
    except ValueError:
        return False, None, "valor_nao_numerico", False

    if cid not in CLASSES:
        return False, None, "classe_invalida", False

    for c in coords:
        if not (MIN_COORD <= c <= MAX_COORD):
            return False, None, "coordenada_fora_de_intervalo", False

    if len(parts) == 5:
        return True, cid, line.strip(), False
    ...
    y2 = min(1.0, max(ys))
    if x2 <= x1 or y2 <= y1:
        return False, None, "poligono_sem_area", False

    xc = (x1 + x2) / 2
    yc = (y1 + y2) / 2
    w = x2 - x1
    h = y2 - y1
    return True, cid, f"{cid} {xc:.6f} {yc:.6f} {w:.6f} {h:.6f}", True
...
def clean_labels_for_images(images: list[Path], labels_dir: Path, cleaned_labels_dir: Path) -> tuple[dict[str, list[int]], LabelStats]:
    ...
    for img in images:
        ...
        for line in lines:
            ...
            ok, cid, cleaned_line, converted = _parse_label_line(stripped)
            if not ok:
                stats.dropped_lines += 1
                file_had_error = True
                continue

            cleaned.append(cleaned_line or stripped)
...
def quality_report(
    ...
    report_lines = [
        ...
        f"Linhas convertidas de poligono para bbox: {stats.converted_lines}",
